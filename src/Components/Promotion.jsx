...
const Promotion = () => {
  ...
  return <section id="promotion">
    <div className="w-full h-auto flex">
      {visibleImages.map((img) => {
        const cleanName = img.name?.replace(/\.[^/.]+$/, "")

        return (
        <div className="w-full h-[85vh] overflow-hidden shrink" key={img.id}>
          <img className="w-full h-full object-cover object-center" src={img.publicUrl} alt={cleanName} width={600} height={600} loading="lazy" />
        </div>
      )})}
    </div>
    <div className='w-full h-full absolute left-0 top-0 px-[5%] sm:px-12 lg:px-20 flex flex-col gap-6 items-start justify-center'>
      <h3 className="text-2xl bg-white font-semibold uppercase text-stone-900 px-2">Walk Your World</h3>
      <p className="bg-white px-2 -mt-3">Built for those who move with purpose.</p>
